...
def validate_request_data(required_fields, optional_fields=None):
    ...
    Args:
        ...
        optional_fields (dict): Dict of optional field names with default values
    """
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            ...
            # Check required fields more safely
            missing_fields = []
...
def login(data):
    ...
    try:
        ...
        # Authenticate user using the User model's authenticate method
        ...
        # Extract user and tokens from successful authentication
        ...
        current_app.logger.error(f"Login error for {email if 'email' in locals() else 'unknown'}: {str(e)}")
        return jsonify({
            'success': False,
...
def update_profile(current_user, data):
    ...
    try:
        ...
        return jsonify({
            ...
            'user': current_user.to_dict(include_timestamps=False)
        }), 200
...
@validate_request_data(required_fields=['username'])
def check_username_availability(data):
    """
